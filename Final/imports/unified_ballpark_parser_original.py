...
Similar to the player parser, but for ballparks
"""
    ...
    try:
        cursor.execute("""
            INSERT INTO ballparks (
                park_name, park_name_en, pf_runs, pf_hr, pf_h, pf_bb,
                games_sample_size
            ) VALUES (?, ?, ?, ?, ?, ?, ?)
        """, (
            ballpark_name,
        ...
        print(f"      HR: {factors['pf_hr']:.3f}")
        print(f"      Hits: {factors['pf_h']:.3f}")
        print(f"      BB: {factors['pf_bb']:.3f}")
